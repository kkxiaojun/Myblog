						...
						<tbody>
								...
								<td>
									<select class="catid" name="articletype">
										<c:forEach items="${typeList}" var="a">
											<c:if test="${a.id==articletype.id}">
												<option value="${a.id}" selected="selected">${a.type}
												</option>
											</c:if>
											<c:if test="${a.id!=articletype.id}">
							...
							<tr class="tr">
								...
								<td><input class="content-title" id="title" name="title"
									size="50" value="${article.title }" type="text"></td>
							</tr>
							<tr class="tr">
								<th>内容：</th>
								<td><textarea name="content" class="content-textarea"
										cols="80" rows="16" placeholder="文章内容...">${article.content }</textarea></td>
							</tr>
							<tr class="tr">
								<th></th>
